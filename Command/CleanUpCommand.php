...
class CleanUpCommand extends Command
{
    ...
    protected function configure()
    {
        $this
            ...
            ->addOption(
                'max-retention-succeeded',
                null,
                InputOption::VALUE_REQUIRED,
                'The maximum retention time for succeeded jobs (value must be parsable by DateTime).',
                '1 hour'
            )
            ->addOption(
                'per-call',
                null,
                InputOption::VALUE_REQUIRED,
                'The maximum number of jobs to clean-up per call.',
                1000
            );
    }
    ...
    {
        $incomingDepsSql = $con->getDatabasePlatform()->modifyLimitQuery(
        ...
        foreach ($this->findExpiredJobs($em, $input) as $job) {
            ...
            if ($result->fetchOne() !== false) {
                $em->getConnection()->transactional(function () use ($em, $job): void {
    ...
    private function findExpiredJobs(EntityManager $em, InputInterface $input): \Generator
    {
        $succeededJobs = function (array $excludedIds) use ($em, $input) {
            return $em->createQuery(
                "SELECT j FROM JMS\JobQueueBundle\Entity\Job j WHERE j.closedAt < :maxRetentionTime AND j.originalJob IS NULL AND j.state = :succeeded AND j.id NOT IN (:excludedIds)"
            )
        ...
        $finishedJobs = function (array $excludedIds) use ($em, $input) {
            return $em->createQuery(
                "SELECT j FROM JMS\JobQueueBundle\Entity\Job j WHERE j.closedAt < :maxRetentionTime AND j.originalJob IS NULL AND j.id NOT IN (:excludedIds)"
            )
        ...
        $canceledJobs = function (array $excludedIds) use ($em, $input) {
            return $em->createQuery(
                "SELECT j FROM JMS\JobQueueBundle\Entity\Job j WHERE j.state = :canceled AND j.createdAt < :maxRetentionTime AND j.originalJob IS NULL AND j.id NOT IN (:excludedIds)"
            )
                ->setParameter('maxRetentionTime', new DateTime('-' . $input->getOption('max-retention')))
    ...
    private function resolveDependencies(EntityManager $em, Job $job): void
    {
        ...
        $em->getConnection()->executeStatement(
            "DELETE FROM jms_job_dependencies WHERE dest_job_id = :id",
            array('id' => $job->getId())
        );
    }
     ...
     * @return Job[]
     */
    private function findStaleJobs(EntityManager $em): \Generator|array
    {
        $excludedIds = array(-1);
        ...
        do {
            ...
            /** @var Job $job */
            $job = $em->createQuery(
                "SELECT j FROM JMS\JobQueueBundle\Entity\Job j
